 ...
 */
 ...
 void dot( double *a, double *b, double *c ) {
    ...
    // Pre-computation of a[i]*b[i] with segments of blockDim.x * gridDim.x values
    double   temp = 0;
    while (tid < N) {
        temp += a[tid] * b[tid];
    ...
    /*< Write your code to compute the sum >*/
/*     // c[cacheIndex] += cache[cacheIndex];
    int i = blockDim.x/2;
    while (i != 0) {
        if (cacheIndex < i)
    ...
    for(int stride = 2; stride <= threadsPerBlock; stride *= 2)
    {
        if (cacheIndex % stride == 0){
            cache[cacheIndex] += cache[cacheIndex + stride/2];
            __syncthreads();
        }
    }
    c[blockIdx.x] = cache[0];
